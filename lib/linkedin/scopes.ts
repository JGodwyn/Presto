...
//
// **Pure on purpose.** The other two modules here are server-only at runtime —
// oauth.ts reads the client secret, publish.ts makes the share call — but the
// Connections page is a client component and needs to know whether a row's
// grant still covers what we now ask for. Importing either of those from a
// client component would pull them into the bundle, which is the trap
// lib/ai/model-constants.ts exists to avoid. Its one import is
// lib/scope-grant.ts, which imports nothing itself.

import { grantCovers, parseGrantedScopes } from "@/lib/scope-grant"

// Re-exported so the LinkedIn side of this reads as one file. It is *defined*
// in lib/scope-grant.ts, shared with lib/x/scopes.ts — the delimiter quirk it
// handles is not LinkedIn's alone.
export { parseGrantedScopes }
...
// `users.read tweet.read tweet.write offline.access` against a list containing
...
// which reconnecting cannot clear. See `isGrantStale` in lib/social-scopes.ts,
// which is what call sites should use.
//
...
export function grantIsCurrent(raw: string): boolean {
  return grantCovers(raw, LINKEDIN_SCOPES)
}
